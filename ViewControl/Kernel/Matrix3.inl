...
//----------------------------------------------------------------------------
inline Matrix3::Matrix3 (Real fM00, Real fM01, Real fM02, Real fM10, Real fM11,
...
//----------------------------------------------------------------------------
inline Matrix3::Matrix3 (const Real afEntry[9], bool bRowMajor)
{
    if (bRowMajor)
    {
...
//----------------------------------------------------------------------------
inline Matrix3::Matrix3 (const Vector3& rkAxis, Real fAngle)
{
...
//----------------------------------------------------------------------------
inline Real Matrix3::operator() (int iRow, int iCol) const
{
    return m_afEntry[iCol+3*iRow];
}
//----------------------------------------------------------------------------
inline Real& Matrix3::operator() (int iRow, int iCol)
{
    return m_afEntry[iCol+3*iRow];
}
//----------------------------------------------------------------------------
...
{
    int i0 = 3*iRow, i1 = i0+1, i2 = i1+1;
    return Vector3(m_afEntry[i0],m_afEntry[i1],m_afEntry[i2]);
}
//----------------------------------------------------------------------------
...
{
    return Vector3(m_afEntry[iCol],m_afEntry[iCol+3],m_afEntry[iCol+6]);
}
//----------------------------------------------------------------------------
inline void Matrix3::GetColumnMajor (Real* afCMajor) const
{
...
inline Matrix3& Matrix3::operator/= (Real fScalar)
{
    ...
    {
        m_afEntry[0] = Math::MAX_REAL;
        m_afEntry[1] = Math::MAX_REAL;
        m_afEntry[2] = Math::MAX_REAL;
        m_afEntry[3] = Math::MAX_REAL;
        m_afEntry[4] = Math::MAX_REAL;
        m_afEntry[5] = Math::MAX_REAL;
        m_afEntry[6] = Math::MAX_REAL;
        m_afEntry[7] = Math::MAX_REAL;
        m_afEntry[8] = Math::MAX_REAL;
    }
...
//----------------------------------------------------------------------------
inline Real Matrix3::QForm (const Vector3& rkU, const Vector3& rkV)
    const
{
...
//----------------------------------------------------------------------------
inline void Matrix3::ToAxisAngle (Vector3& rkAxis, Real& rfAngle) const
{
        ...
        else
        {
            ...
            if (m_afEntry[0] >= m_afEntry[4])
            {
                ...
                if (m_afEntry[0] >= m_afEntry[8])
                {
                    ...
                    rkAxis[0] = ((Real)0.5)*Math::Sqrt(m_afEntry[0] -
                ...
                else
                {
                    ...
                    rkAxis[2] = ((Real)0.5)*Math::Sqrt(m_afEntry[8] -
            ...
            else
            {
                ...
                if (m_afEntry[4] >= m_afEntry[8])
                {
                    ...
                    rkAxis[1] = ((Real)0.5)*Math::Sqrt(m_afEntry[4] -
...
inline void Matrix3::Orthonormalize ()
{
    ...
    // compute q0
    Real fInvLength = Math::InvSqrt(m_afEntry[0]*m_afEntry[0] +
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::FromEulerAnglesXYZ (Real fXAngle, Real fYAngle,
    Real fZAngle)
{
    Real fCos, fSin;

    fCos = Math::Cos(fXAngle);
    fSin = Math::Sin(fXAngle);
    Matrix3 kXMat(
    ...
    fCos = Math::Cos(fZAngle);
    fSin = Math::Sin(fZAngle);
    Matrix3 kZMat(
        fCos,-fSin,(Real)0.0,
        fSin,fCos,(Real)0.0,
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::FromEulerAnglesXZY (Real fXAngle, Real fZAngle,
    Real fYAngle)
{
    Real fCos, fSin;
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::FromEulerAnglesYXZ (Real fYAngle, Real fXAngle,
    Real fZAngle)
{
    Real fCos, fSin;
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::FromEulerAnglesYZX (Real fYAngle, Real fZAngle,
    Real fXAngle)
{
    Real fCos, fSin;
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::FromEulerAnglesZXY (Real fZAngle, Real fXAngle,
    Real fYAngle)
{
    Real fCos, fSin;
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::FromEulerAnglesZYX (Real fZAngle, Real fYAngle,
    Real fXAngle)
{
    Real fCos, fSin;
...
//----------------------------------------------------------------------------
inline bool Matrix3::ToEulerAnglesXYZ (Real& rfXAngle, Real& rfYAngle,
    Real& rfZAngle) const
{
    // rot =  cy*cz          -cy*sz           sy
...
//----------------------------------------------------------------------------
inline bool Matrix3::ToEulerAnglesXZY (Real& rfXAngle, Real& rfZAngle,
    Real& rfYAngle) const
{
    // rot =  cy*cz          -sz              cz*sy
...
//----------------------------------------------------------------------------
inline bool Matrix3::ToEulerAnglesYXZ (Real& rfYAngle, Real& rfXAngle,
    Real& rfZAngle) const
{
...
//----------------------------------------------------------------------------
inline bool Matrix3::ToEulerAnglesYZX (Real& rfYAngle, Real& rfZAngle,
    Real& rfXAngle) const
{
...
//----------------------------------------------------------------------------
inline bool Matrix3::ToEulerAnglesZXY (Real& rfZAngle, Real& rfXAngle,
    Real& rfYAngle) const
{
...
//----------------------------------------------------------------------------
inline bool Matrix3::ToEulerAnglesZYX (Real& rfZAngle, Real& rfYAngle,
    Real& rfXAngle) const
{
...
//----------------------------------------------------------------------------
inline Matrix3& Matrix3::Slerp (Real fT, const Matrix3& rkR0,
    const Matrix3& rkR1)
{
    Vector3 kAxis;
    Real fAngle;
...
//----------------------------------------------------------------------------
inline bool Matrix3::Tridiagonalize (Real afDiag[3], Real afSubd[2])
{
...
//----------------------------------------------------------------------------
inline bool Matrix3::QLAlgorithm (Real afDiag[3], Real afSubd[2])
{
...
inline void Matrix3::GolubKahanStep (Matrix3& rkA, Matrix3& rkL, Matrix3& rkR)
{
    ...
    Real fDiscr = Math::Sqrt(fDiff*fDiff+((Real)4.0)*fT12*fT12);
    Real fRoot1 = ((Real)0.5)*(fTrace+fDiscr);
    Real fRoot2 = ((Real)0.5)*(fTrace-fDiscr);

    // adjust right
    Real fY = rkA[0][0] - (Math::FAbs(fRoot1-fT22) <=
